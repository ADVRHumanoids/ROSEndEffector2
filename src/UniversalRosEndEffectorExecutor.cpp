...
ROSEE::UniversalRosEndEffectorExecutor::UniversalRosEndEffectorExecutor ( std::string ns ) {
    ...
    RCLCPP_INFO_STREAM (_node->get_logger(), "Fingers in EEInterface: " );
    for ( auto& f : _ee->getFingers() ) {
        RCLCPP_INFO_STREAM (_node->get_logger(), f );
    }
    ...
    _ros_service_handler = std::make_shared<RosServiceHandler>(_node, mapActionHandlerPtr, folderForActions+"/generics/");
    _ros_service_handler->init(_ee->getFingers().size());
    
    // actions
    std::string actionGraspingCommandName;
    _node->declare_parameter("rosAction_grasping_command", "action_command");
    _node->get_parameter("rosAction_grasping_command", actionGraspingCommandName);
    _ros_action_server = std::make_shared<RosActionServer> (actionGraspingCommandName , _node);
...
bool ROSEE::UniversalRosEndEffectorExecutor::init_motor_reference_pub() {
    ...
    _motor_reference_pub = _node->create_publisher<sensor_msgs::msg::JointState> ( motor_reference_topic, motor_reference_queue );
...
bool ROSEE::UniversalRosEndEffectorExecutor::init_grasping_primitive() {
    ...
    RCLCPP_INFO_STREAM (_node->get_logger(), "PINCHES-TIGHT:" );
    for ( auto &i : _pinchParsedMap ) {
        i.second->print();
    }
    RCLCPP_INFO_STREAM (_node->get_logger(), "PINCHES-LOOSE:" );
    for ( auto &i : _pinchLooseParsedMap ) {
        i.second->print();
    }
    RCLCPP_INFO_STREAM (_node->get_logger(), "TRIGGERS:" );
    for ( auto &i : _trigParsedMap ) {
        i.second->print();
    }
    RCLCPP_INFO_STREAM (_node->get_logger(), "TIP FLEX:" );
    for ( auto &i : _tipFlexParsedMap ) {
        i.second->print();
    }
    RCLCPP_INFO_STREAM (_node->get_logger(), "FINGER FLEX:" );
    ...
    // recap
    RCLCPP_INFO_STREAM (_node->get_logger(), "GRASP:" );
...
void ROSEE::UniversalRosEndEffectorExecutor::init_joint_state_sub () {
    ...
    RCLCPP_INFO_STREAM (_node->get_logger(), "Getting joint pos from '" << topic_name_js << "'" );
    
    _joint_state_sub = _node->create_subscription<sensor_msgs::msg::JointState>(topic_name_js, 1, 
                                 std::bind(&ROSEE::UniversalRosEndEffectorExecutor::joint_state_clbk, this, _1));
}

void ROSEE::UniversalRosEndEffectorExecutor::joint_state_clbk(const sensor_msgs::msg::JointState::SharedPtr msg) {
...
bool ROSEE::UniversalRosEndEffectorExecutor::updateGoal() {
    ...
    rosee_msg::msg::ROSEEActionControl goal = _ros_action_server->getGoal();
    
    timed_requested = false;
    
    if (goal.action_type != ROSEE::Action::Type::Timed){
        if (goal.percentage < 0 || goal.percentage > 1) { 
            RCLCPP_ERROR_STREAM (_node->get_logger(), "Received an action-goal with percentage " << goal.percentage << 
    ...
    {
        RCLCPP_ERROR_STREAM (_node->get_logger(), "Received an action-goal of type None (" << goal.action_type << 
    ...
    default : {
        RCLCPP_ERROR_STREAM (_node->get_logger(), "Received an action-goal of type " << goal.action_type << 
...
bool ROSEE::UniversalRosEndEffectorExecutor::readOptionalCommands(
    ...
    if (motors_names.size() != motors_commands.size() &&
        ...
        RCLCPP_ERROR_STREAM (_node->get_logger(), "In receiving the goal command, the optional field is formed badly: " 
    ...
    for (int i=0; i<motors_names.size(); i++) {
        ...
        if( id >= 0 ) {
            ...
            RCLCPP_WARN_STREAM (_node->get_logger(), "Trying to send an optional command to motor: " << motors_names.at(i) 
    ...
    for ( auto it : _motor_involved_mask ) {
        ...
        if ( it.second  != 0 ) {
            ...
            if( id >= 0 ) {
                ...
                RCLCPP_WARN_STREAM (_node->get_logger(), "Trying to move Joint: " << it.first << " with ID: " << id );
    ...
    for ( auto it : _motor_involved_mask ) {
        ...
        if ( it.second  != 0 ) {
            ...
            if( id >= 0 ) {
                ...
                RCLCPP_ERROR_STREAM (_node->get_logger(), "YOU SHOULD NOT BE HERE, previous error should stop execution");
            }
        }
    }
...
bool ROSEE::UniversalRosEndEffectorExecutor::publish_motor_reference() {
    ...
    _mr_msg.header.stamp = rclcpp::Clock().now();
...
void ROSEE::UniversalRosEndEffectorExecutor::timer_callback ( ) {
    ...
    if (timed_requested) {
        if (timer.elapsed_time<double, std::chrono::milliseconds>()  >  msToWait )  {
            ...
            RCLCPP_INFO_STREAM (_node->get_logger(),"Waiting time to execute timed action...");
        }
...
void ROSEE::UniversalRosEndEffectorExecutor::spin() {
    ...
    RCLCPP_INFO_STREAM (_node->get_logger(), "Started looping @ " << 1./_period << "Hz" );
